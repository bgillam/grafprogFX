...
import GrafProg.GrafObjects.Function.FunctionStringErrorChecks;
...
public class TableColumnGeneratorController {
    ...
    private boolean generateRecursiveValues(){
        ...
        //else if (!GrafProg.GrafObjects.Function.FunctionString.isValidAtXIgnoreDomainError(functionString, 1)) {GrafProg.GrafProg.GrafUtils.GrafInputHelpers.setTextFieldColor(t2, "red");  ok = false;  }
        ...
        for (int row = beginRow; row <= endRow; row++) {
           // try {
                ...
                anMinus1=newVal;}
          //  } catch (GrafProg.GrafProg.GrafUtils.DomainViolationException e) { }  //we know we should not get an exception because tested previously
         //   catch (GrafProg.GrafObjects.Function.FunctionFormatException e) {}
        //myDaddy.repaint();
        return true;
    }
    ...
    private boolean generateFunctionValues(){
        ...
        //if (!GrafProg.GrafObjects.Function.FunctionString.isValidAtXIgnoreDomainError(t1.getText(), 1)) {GrafProg.GrafProg.GrafUtils.GrafInputHelpers.setTextFieldColor(rowStartText, "red");  ok = false;  }
        ...
        for (int row = beginRow; row <= endRow; row++)
            ...
            //catch (GrafProg.GrafProg.GrafUtils.DomainViolationException e) { }   //we know we will not get an exception because tested in previous if statement
            //catch (GrafProg.GrafObjects.Function.FunctionFormatException e) {}
        return true;
        //myDaddy.repaint();
    }
